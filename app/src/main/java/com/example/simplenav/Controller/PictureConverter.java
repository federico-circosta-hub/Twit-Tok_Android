...
public class PictureConverter {
    ...
    public static Bitmap createImage(String pic) {
        byte[] imageBytes;
        if (pic != null && isBase64Encoded(pic)) {
            imageBytes = Base64.decode(pic, Base64.DEFAULT);
        } else {
            imageBytes = Base64.decode(NONE, Base64.DEFAULT);
        }
        /*        if (bp == null) {
            imageBytes = Base64.decode(FAILED, Base64.DEFAULT);
    ...
    private static boolean isBase64Encoded(String s) {
        return (s.length() % 4 == 0) && (s.length() > 1000) && (s.length() < 137000) && !(s.contains("base64"));
    }
}
